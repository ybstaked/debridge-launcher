...
package keystore // import "github.com/debridge-finance/orbitdb-go/pkg/berty.tech/go-ipfs-log/keystore"
...
func (k *Keystore) HasKey(id string) (bool, error) {
	...
	if ok == false {
		value, err := k.store.Get(k.ctx, datastore.NewKey(id))
		if err != nil {
...
func (k *Keystore) CreateKey(id string) (crypto.PrivKey, error) {
	...
	if err := k.store.Put(k.ctx, datastore.NewKey(id), keyBytes); err != nil {
...
func (k *Keystore) GetKey(id string) (crypto.PrivKey, error) {
	...
	if !ok || cachedKey == nil {
		keyBytes, err = k.store.Get(k.ctx, datastore.NewKey(id))
